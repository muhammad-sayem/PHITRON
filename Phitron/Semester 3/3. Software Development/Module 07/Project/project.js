const loadData = (global) => {
    const searchText = document.getElementById("insert-input").value;
    console.log(searchText);
    fetch(`https://www.themealdb.com/api/json/v1/1/search.php?f=${searchText ? searchText : global}`)
        .then(res => res.json())
        .then(data => console.log(displayData(data.meals)));
};
...
const displayData = (data) => {
    ...
    data.forEach(meal => {
        ...
        card.classList.add("box");      // card er class name disi jate css diye thik kora jaay //
        card.innerHTML = `
